...
public class Problema {
	...
	public CromosomaAsigC evaluacion(CromosomaAsigC[] pob) {
		...
		double maxApt = 0, puntAcomulada = 0; // Se debe calcular fuera, y
												// entrar como parametro de la
												// funcion
		double sum = 0;
		double sumDefault = 0;
	...
	private void cruceOX(CromosomaAsigC new1, CromosomaAsigC new2) {
		...
		while (!pos1.isEmpty() || !pos2.isEmpty()) {
			...
			// Dejamos los numeros originales que no generan conflicto (Primer
			// Hijo)
			if (pos1.contains(ii) && !pos1.isEmpty()) { // Si la posicion ii
														// esta disponible
				int val = parent1[jj];
			...
			// Dejamos los numeros originales que no generan conflicto (Segundo
			// Hijo)
			if (pos2.contains(k) && !pos2.isEmpty()) { // Si la posicion k esta
														// disponible
				int val = parent2[h];
	...
	private void backtracking(ArrayList<Integer> pos, ArrayList<Integer> valores, ArrayList<Integer> mutado) {
